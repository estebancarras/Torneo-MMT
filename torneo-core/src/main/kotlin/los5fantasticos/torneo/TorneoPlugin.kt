...
class TorneoPlugin : JavaPlugin() {
    ...
    override fun onDisable() {
        ...
        minigameModules.forEach { module ->
            try {
                module.onDisable()
                logger.info("✓ ${module.name} deshabilitado")
            } catch (e: Exception) {
                logger.severe("✗ Error al deshabilitar ${module.name}: ${e.message}")
                e.printStackTrace()
            }
        }
    ...
    fun registerMinigame(module: MinigameModule) {
        try {
            ...
            logger.info("✓ Minijuego cargado: ${module.name} v${module.version}")
            logger.info("  Descripción: ${module.description}")
        } catch (e: Exception) {
            logger.severe("✗ Error al registrar minijuego ${module.name}: ${e.message}")
            e.printStackTrace()
        }
    }
    ...
    fun getMinigame(name: String): MinigameModule? {
        return minigameModules.find { it.name.equals(name, ignoreCase = true) }
    }
